...
- 节点面板按基础、流程控制、并行执行分组，28px 行内分列显示中文名称与英文类型。结构树同为 28px 单行，层级缩进 12px，右侧标注有效直接子节点数；动作类型保留在悬浮提示，不重复占第二行。两者共用 `workbench.css` 的无边框选中/悬停样式。
...
悬浮提示统一使用 `--ui-tooltip-bg/text/shadow`：深色中性灰底和柔灰文字，浅色近白底和深灰文字；无描边、4px 圆角、3px 下移的轻阴影。桌面、独立窗口和旧编辑器共用，避免局部样式恢复蓝灰亮边。
