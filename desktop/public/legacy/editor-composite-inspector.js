 ...
 * 主文件通过 window.StudioEditorCompositeInspector({ ... }) 注入依赖。
 * 注意：函数保持 2 空格缩进，composite-inspector / decorator-inputs 测试按函数名切片执行。
 */
(() => {
  'use strict';

  window.StudioEditorCompositeInspector = function StudioEditorCompositeInspector(deps) {
  const { el, section, field, selectInput, checkbox, segmentedInput, textInput, iconButton, addRowButton, conditionControl, conditionOperandControl, conditionParseLiteral, nodeChildrenOptions, nodeById, mutate, disconnect, runtimeInstanceLabel, removeInstanceRun, workflowInputs, render, state, decoratorLabel, isBindingValue, clone, allRefs, referenceLabel, valueBindingMenu, toast } = deps;
  ...
  return { renderCompositeInspector, renderDecorators };
  };
})();
