(() => {
  ...
  const StudioSidebarState = window.StudioEditorSidebarState({ state, collectNodeCardVariableRefs, nodes, currentInspectorSelection, vscode });
  const postSidebarState = (...args) => StudioSidebarState.postSidebarState(...args);
  ...
  const StudioToolbar = window.StudioEditorToolbar({ state, $, el, UI, vscode, showMenu, zoomAt, setDirty, toast, nodes, focusNode });
  const renderInstancePicker = (...args) => StudioToolbar.renderInstancePicker(...args);
  const renderWorkflowPicker = (...args) => StudioToolbar.renderWorkflowPicker(...args);
  const renderWorkflowBreadcrumb = (...args) => StudioToolbar.renderWorkflowBreadcrumb(...args);
  const bindToolbar = (...args) => StudioToolbar.bindToolbar(...args);
  const searchNodeByName = (...args) => StudioToolbar.searchNodeByName(...args);
  ...
  const StudioSchema = window.StudioEditorSchema();
  const definitionSchema = (...args) => StudioSchema.definitionSchema(...args);
  const compatibleRefType = (...args) => StudioSchema.compatibleRefType(...args);
  const appendNestedRefs = (...args) => StudioSchema.appendNestedRefs(...args);
  ...
  const StudioCompositeInspector = window.StudioEditorCompositeInspector({ el, section, field, selectInput, checkbox, segmentedInput, textInput, iconButton, addRowButton, conditionControl, conditionOperandControl, conditionParseLiteral, nodeChildrenOptions, nodeById, mutate, disconnect, runtimeInstanceLabel, removeInstanceRun, workflowInputs, render, state, decoratorLabel, isBindingValue, clone, allRefs, referenceLabel, valueBindingMenu, toast });
  const renderCompositeInspector = (...args) => StudioCompositeInspector.renderCompositeInspector(...args);
  const renderDecorators = (...args) => StudioCompositeInspector.renderDecorators(...args);
  ...
  const StudioExport = window.StudioEditorExport({ state, graph, vscode, bounds, wrap, toast, NS });
  const exportFullCanvasImage = (...args) => StudioExport.exportFullCanvasImage(...args);
  const setExportBusy = (...args) => StudioExport.setExportBusy(...args);
  const collectExportTemplatePaths = (...args) => StudioExport.collectExportTemplatePaths(...args);
  const applyInlineThumbnails = (...args) => StudioExport.applyInlineThumbnails(...args);
  ...
  const StudioTemplateCheck = window.StudioEditorTemplateCheck({ state, $, el, nodeById, catalogByName, clone, vscode, toast });
  const requestTemplateCheck = (...args) => StudioTemplateCheck.requestTemplateCheck(...args);
  const closeTemplateCheck = (...args) => StudioTemplateCheck.closeTemplateCheck(...args);
  const renderTemplateCheck = (...args) => StudioTemplateCheck.renderTemplateCheck(...args);

  const StudioAssetBrowser = window.StudioEditorAssetBrowser({ state, $, el, mutate, nodeById, toast, vscode, requestRoi });
  const openAssetBrowser = (...args) => StudioAssetBrowser.openAssetBrowser(...args);
  const closeAssetBrowser = (...args) => StudioAssetBrowser.closeAssetBrowser(...args);
  const renderAssetBrowser = (...args) => StudioAssetBrowser.renderAssetBrowser(...args);
  const restoreAssetBrowserAfterRoi = (...args) => StudioAssetBrowser.restoreAssetBrowserAfterRoi(...args);
  const requestTemplateReplacement = (...args) => StudioAssetBrowser.requestTemplateReplacement(...args);

  const StudioWorkflowBrowser = window.StudioEditorWorkflowBrowser({ state, $, el, nodeById, mutate, toast });
  const workflowReference = (...args) => StudioWorkflowBrowser.workflowReference(...args);
  const openWorkflowBrowser = (...args) => StudioWorkflowBrowser.openWorkflowBrowser(...args);
  const closeWorkflowBrowser = (...args) => StudioWorkflowBrowser.closeWorkflowBrowser(...args);

  const StudioRoiPicker = window.StudioEditorRoiPicker({ state, $, el, mutate, vscode, toast, nodeById, restoreAssetBrowserAfterRoi });
  const openRoiPicker = (...args) => StudioRoiPicker.openRoiPicker(...args);
