...
function shellHarness(options = {}) {
  ...
  const ctrl = createDeleteShortcuts({
    matchesShortcut: (event) => (event.key === 'Delete' || event.key === 'Backspace')
  ...
  if (options.deleteTarget) ctrl.setDeleteTarget(options.deleteTarget);
...
test('Delete 与 Backspace 都能删除内容浏览器里选中的条目', () => {
  for (const key of ['Delete', 'Backspace']) {
    ...
    assert.equal(h.ctrl.handleDeleteShortcut(event), true, key);
    assert.equal(event.defaultPrevented, true, key);
    assert.deepEqual(h.calls.deleted, [workflowItem], key);
    assert.equal(h.ctrl.getDeleteTarget(), undefined, `${key} 之后不应保留过期目标`);
  }
});
...
test('输入控件与修饰键不抢占原生删除语义', () => {
  ...
  input.editable = true;
  assert.equal(editable.ctrl.handleDeleteShortcut(keyEvent('Delete', {target: input})), false);
  ...
  assert.equal(modified.ctrl.handleDeleteShortcut(keyEvent('Delete', {ctrlKey: true})), false);
  assert.equal(modified.ctrl.handleDeleteShortcut(keyEvent('Backspace', {altKey: true})), false);
  ...
  assert.equal(modal.ctrl.handleDeleteShortcut(keyEvent('Delete')), false);
  const picker = shellHarness({deleteTarget: {kind: 'content', path: 'workflows/a.json'}, entries: [workflowItem], roiPickerState: {busy: false}});
  assert.equal(picker.ctrl.handleDeleteShortcut(keyEvent('Delete')), false);
});
...
test('目录树与面包屑里的文件夹可以删除，项目根目录被拦截', () => {
  ...
  assert.equal(folder.ctrl.handleDeleteShortcut(keyEvent('Delete')), true);
  ...
  for (const rootFolder of ['assets', 'workflows']) {
    ...
    assert.equal(protectedRoot.ctrl.handleDeleteShortcut(keyEvent('Delete')), true);
...
test('执行队列行按 Delete 移出队列，运行中只提示不改队列', () => {
  ...
  assert.equal(idle.ctrl.handleDeleteShortcut(event), true);
  assert.equal(event.defaultPrevented, true);
  assert.deepEqual(idle.calls.queue, [['workflows/a.json', false]]);
  assert.deepEqual(idle.calls.queueRows, ['']);
  assert.equal(idle.ctrl.getDeleteTarget(), undefined);
  ...
  assert.equal(running.ctrl.handleDeleteShortcut(keyEvent('Delete')), true);
...
test('结构树与变量列表的选中项经画布删除', () => {
  ...
  const event = keyEvent('Delete');
  assert.equal(h.ctrl.handleDeleteShortcut(event), true);
  assert.equal(event.defaultPrevented, true);
...
test('没有登记目标或目标已失效时不吃按键', () => {
  ...
  const event = keyEvent('Delete');
  assert.equal(empty.ctrl.handleDeleteShortcut(event), false);
  assert.equal(event.defaultPrevented, false);

  const stale = shellHarness({deleteTarget: {kind: 'content', path: 'workflows/gone.json'}, selectedContentPath: ''});
  assert.equal(stale.ctrl.handleDeleteShortcut(keyEvent('Delete')), false);
  assert.equal(stale.calls.deleted.length, 0);
  assert.equal(stale.ctrl.getDeleteTarget(), undefined);

  const staleQueue = shellHarness({deleteTarget: {kind: 'queue', rel: 'workflows/gone.json'}, overviewSelection: []});
  assert.equal(staleQueue.ctrl.handleDeleteShortcut(keyEvent('Backspace')), false);
